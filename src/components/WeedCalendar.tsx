...
export default function WeedCalendar() {
  const now = new Date();
  ...
  const byDay = useMemo(() => {
    ...
    for (const s of allSessions) {
      const key = todayLocal(new Date(s.created_at));
  ...
  const selectedSessions = useMemo(
    () => allSessions.filter((s) => todayLocal(new Date(s.created_at)) === selected),
          ...
          {cells.map((day, i) => {
            ...
            return (
              ...
              >
                ...
                {count > 0 && (
                  <span
                    className={`mt-0.5 h-1.5 w-1.5 rounded-full ${isSelected ? "bg-white" : ""}`}
